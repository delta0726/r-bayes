...
file_beer_sales_ab <- read_csv("csv/2-6-1-beer-sales-ab.csv")
...
# --- 正規分布に従う事前分布を指定
mcmc_result_2 <-
  stan(file = "stan/2-6/2-6-1-normal-prior.stan",
       data = data_list,
       seed = 1)
...
# --- 乱数の生成(対数密度加算文の使用)
mcmc_result_3 <-
  stan(file = "stan/2-6/2-6-2-lp.stan",
       data = data_list,
       seed = 1)
...
mcmc_result_4 <-
  stan(file = "stan/2-6/2-6-3-lp-normal-prior.stan",
       data = data_list,
       seed = 1)
...
# --- 対数密度加算文の使用、ベクトル化
mcmc_result_5 <-
  stan(file = "stan/2-6/2-6-4-lp-normal-prior-vec.stan",
       data = data_list,
       seed = 1)
...
# --- 乱数の生成
mcmc_result_6 <-
  stan(file = "stan/2-6/2-6-5-difference-mean.stan",
       data = data_list_ab,
       seed = 1)
